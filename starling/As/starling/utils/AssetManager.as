package starling.utils
{
    ...
    public class AssetManager extends EventDispatcher
    {
        ...
        private var mVerbose:Boolean = true;
        ...
        public function addObject(name:String, obj:Object):void
        {
           ...
           for each (var rawAsset:Object in rawAssets)
           {
               if (rawAsset is Array)
               {
                  ...
                  var a:Array = rawAsset as Array;
                	for each (var child:Object in a) {
                		this.enqueue(child);
                	}
              // }
              ...
              //     for each (childNode in typeXml.variable.(@type == "Class"))
               ...
               }else if (getQualifiedClassName(rawAsset) == "flash.filesystem::File")
               {
                    ...
                    else if (!file.isHidden)
                    {
                        if (file.isDirectory)
                        {
	                        ...
	                        for each (var child:flash.filesystem.File in files) {
		                        if(child != null){
               ...
               {
               	trace("enque with string", rawAsset);
                    enqueueWithName(rawAsset);
               }
               else
               {
                                         ...
                                         onProgress:Function, onComplete:Function):void
        {
           ...
           process = function (asset:Object):void
           {
               ...
               else if (asset is Bitmap)
               {
                   ...
                   {
                       mNumLostTextures++;
                       loadRawAsset(rawAsset, null, function(innerAsset:Object):void
                       {
                           try { texture.root.uploadBitmap(innerAsset as Bitmap); }
                           catch (e:Error) { log("Texture restoration failed: " + e.message); }
                           
                           innerAsset.bitmapData.dispose();
               ...
               else if (asset is ByteArray)
               {
                   ...
                   if (AtfData.isAtfData(bytes))
                   {
                       ...
                       texture.root.onRestore = function():void
                       {
                           ...
                           loadRawAsset(rawAsset, null, function(innerAsset:Object):void
                           {
                               try { texture.root.uploadAtfData(innerAsset as ByteArray, 0, true); }
                               catch (e:Error) { log("Texture restoration failed: " + e.message); }
                   ...
                   {
                       try { obj = JSON.parse(bytes.readUTFBytes(bytes.length)); }
                       catch (e1:Error)
                       {
                           log("Could not parse JSON: " + e1.message);
                           dispatchEventWith(Event.PARSE_ERROR, false, name);
                       }
		   
                       if (obj) addObject(name, obj);
                   ...
                   else if (byteArrayStartsWith(bytes, "<"))
                   {
                       ...
                       {
                           log("Could not parse XML: " + e2.message);
                           dispatchEventWith(Event.PARSE_ERROR, false, name);
                       }
        ...
        /**his method is called internally for each element of the queue when it is loaded.
         ...
         * an work with, e.g. a Bitmap, a Sound, XML data, or a ByteArray. This object needs to
         * e passed to the 'onComplete' callback.</p>
         *
         * p>The calling method will then process this data accordingly (e.g. a Bitmap will be
         * ransformed into a texture). Unknown types will be available via 'getObject()'.</p>
         *
         * p>When overriding this method, you can call 'onProgress' with a number between 0 and 1
         * o update the total queue loading progress.</p>
         */
        procted function loadRawAsset(rawAsset:Object, onProgress:Function, onComplete:Function):void
        {
            ...
            onHttpResponseStatus = function (e:HTTPStatusEvent):void
            {
                if (extension == null)
                {
                    var headers:Array = e["responseHeaders"];
                    var contentType:String = getHttpHeader(headers, "Content-Type");
                    var regexp:RegExp =  /(audio|image)/;
                    if (contentType && regexp.exec(contentType))
                        extension = contentType.split("/").pop();
                }
            };
		   
           onLoadProgress =  function (e:ProgressEvent):void
            {
                if (onProgress != null && e.bytesTotal > 0)
                    onProgress(e.bytesLoaded / e.bytesTotal);
            };
            
           onUrlLoaderComplete = function (e:Object):void
            {
           ...
           onLoaderComplete =  function (e:Object):void
            {
                urlLoader.data.clear();
                complete(e.target.content);
            };
            ...
            else if (rawAsset is String)
            {
                ...
                urlLoader.load(new URLRequest(url), true);
            }
        }
        ...
        private function byteArrayStartsWith(bytes:ByteArray, c:String):Boolean
        {
            var start:int = 0;
            var length:int = bytes.length;
            var wanted:int = c.charCodeAt(0);
            ...
            {
                var b:int = bytes[i];
                if (b == 0 || b == 10 || b == 13 || b == 32) continue; // null, \n, \r, space
                else return b == wanted;
            }
