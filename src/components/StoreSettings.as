...
//            _content.addChild(_resetBtn);
    ...
    private function onAlarmBtn(event:TouchEvent):void {
        if (_alarmBtn.touchIsEnd(event)) {
            if (AppProperties.isIOS) {
    ...
    private function onInstrumentsBtn(event:TouchEvent):void {
        if (_instrumentsBtn.touchIsEnd(event)) {
            if (AppProperties.isIOS) {
    ...
    private function onPolarBtn(event:TouchEvent):void {
        if (_polarBtn.touchIsEnd(event)) {
            if (AppProperties.isIOS) {
    ...
    private function onRestoreBtn(event:TouchEvent):void {
        if (_restoreBtn.touchIsEnd(event)) {
            if (AppProperties.isIOS) {
    ...
    private function enabledHandler(event:Event):void {
        setButtonPurchaseState();
    }

    private function iosStoreProductInfosReadyHandler(event:Event):void {
        loadPricesForButtonsFromIOSStore();
    }
    ...
    private function androidStoreProductInfosReadyHandler(event:Event):void {
        loadPricesForButtonsFromAndroidStore();
    }
    ...
    private function storeProductInfosFailedHandler(event:Event):void {
    ...
    private function resetBtn_touchHandler(event:TouchEvent):void {
        if (_resetBtn.touchIsEnd(event)) {
            EdoMobile.reset();
        }
    }

    private function product_infos_failedHandler(event:Event):void {
