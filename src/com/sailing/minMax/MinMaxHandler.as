...
public class MinMaxHandler extends EventDispatcher implements IParserListener {
    ...
    public function MinMaxHandler() {
        if (_instance != null) {
            ...
            initListener();
            UnitHandler.instance.addEventListener(UnitChangedEvent.CHANGE, unit_changed_eventHandler);
        }
    }
    ...
    private function initListener():void {
        for (var i:int = 0; i < _predefKeys.length; i++) {
            ...
            _listeners[s[0]][s[1]] = {global: new MinMax(undefined, undefined, null, GeneralUtils.getClass(SailData.actualSailData[s[0]][s[1]])), instruments: []};
        }
    }
    ...
    public function updateMinMax(key:String, data:BaseSailData):void {
        ...
        for (var skey:String in l) {
            ...
            for (var i:int = 0; i < l[skey].instruments.length; i++) {
                instrument = l[skey].instruments[i] as BaseInstrument;
                if ((instrument.minMaxVars[key + "." + skey] as MinMax).setMinMaxLimit((data[skey] is Unit) ? data[skey].value : data[skey]) && instrument.enabled) {
                    try {
                        instrument.minMaxChanged();
...
//        updateMinMax(key, (CommonStore.instance.isInstrumentsEnabled) ? SailData.actualSailData[key] : InstrumentHandler.previewSailData[key]);
        updateMinMax(key, SailData.actualSailData[key]);
    }
