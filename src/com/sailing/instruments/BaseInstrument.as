...
//        CommonStore.instance.addEventListener("polar-enabled", enabledHandler);
    ...
    public function get enabled():Boolean {
        return (this is Polar || this is Perf1 || this is Perf3 || this is PolarControl) ? CommonStore.instance.isPolarEnabled : CommonStore.instance.isInstrumentsEnabled;
    }
...
//    }
    ...
    private function onUnitChange(event:UnitChangedEvent):void {
        unitChanged();
    }

    private function onScale(event:ScaleTouchEvent):void {
//        if(this==event.data.target) {
//            this.width += event.data.scale;
//            this.height += event.data.scale;
//        }
    }

    private function onEnd(event:ScaleTouchEvent):void {
//        if(this==event.data.target) {
...
//        }
    ...
    protected function touchIsEnd(event:TouchEvent):Touch {
        var began:Touch = event.getTouch(this, TouchPhase.BEGAN);
        var ended:Touch = event.getTouch(this, TouchPhase.ENDED);
        if(began) {
