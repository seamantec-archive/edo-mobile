...
public class CloudHandler extends EventDispatcher {
    ...
    private var _saveLogFileStatus:int;
...
//            PolarFileHandler.instance.deleteAll();
    ...
    public function saveLogFile(name:String, file:File):void {
        if(!signedIn){
            return;
        }
        ...
        var upload:Multipart = new Multipart(serverEndpoint + "save_log_file");
        upload.addFile("file", bytes, "application/zip", "edo_nmea.edoz");

        var saveLogFileRequest:URLRequest = upload.request;
        var length:int = _headers.length;
        for (var i:int = 0; i < length; i++) {
            if (_headers[i].name != "Content-Type") {
                saveLogFileRequest.requestHeaders.push(_headers[i]);
            }
        }

        var loader:URLLoader = new URLLoader();
        loader.addEventListener(Event.COMPLETE, onSaveLogFileComplete, false, 0, true);
        loader.addEventListener(HTTPStatusEvent.HTTP_RESPONSE_STATUS, onSaveLogFileStatus);
        loader.addEventListener(SecurityErrorEvent.SECURITY_ERROR, onSaveLogFileError, false, 0, true);
        loader.addEventListener(IOErrorEvent.IO_ERROR, onSaveLogFileError, false, 0, true);
        loader.load(saveLogFileRequest);
    }

    private function onSaveLogFileStatus(event:HTTPStatusEvent):void {
        _saveLogFileStatus = event.status;
    }

    private function onSaveLogFileComplete(event:Event):void {
        var loader:URLLoader = event.target as URLLoader;
        loader.removeEventListener(HTTPStatusEvent.HTTP_RESPONSE_STATUS, onSaveLogFileStatus);
        loader.removeEventListener(Event.COMPLETE, onSaveLogFileComplete);
        loader.removeEventListener(SecurityErrorEvent.SECURITY_ERROR, onSaveLogFileError);
        loader.removeEventListener(IOErrorEvent.IO_ERROR, onSaveLogFileError);

        if (_saveLogFileStatus == 200) {
            NmeaLogger.instance.deleteLogFile();
        }
    }

    private function onSaveLogFileError(event:Event):void {
        var loader:URLLoader = event.target as URLLoader;
        loader.removeEventListener(HTTPStatusEvent.HTTP_RESPONSE_STATUS, onSaveLogFileStatus);
        loader.removeEventListener(Event.COMPLETE, onSaveLogFileComplete);
        loader.removeEventListener(SecurityErrorEvent.SECURITY_ERROR, onSaveLogFileError);
