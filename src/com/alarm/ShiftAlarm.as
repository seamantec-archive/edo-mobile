...
public class ShiftAlarm extends LimitAlarm {
    ...
    public override function set actualValue(value:Number):void {
        ...
        gotValue = value;
        _lastTimestamp = SailData.actualSailData.sailDataTimestamp;
...
//        }
    ...
    override public function resetLockedValue():void {
        if (!SailData.actualSailData[_listenerKey.key].isValid()) {
            return;
        }
        if (SailData.actualSailData[_listenerKey.key][_listenerKey.parameter] is Unit) {
            startValue = SailData.actualSailData[_listenerKey.key][_listenerKey.parameter].value;
        } else {
            startValue = SailData.actualSailData[_listenerKey.key][_listenerKey.parameter]
        }
        _lastSaidValue = 0;
        calculateActualValue();
    }
