...
public class AlarmHandler extends EventDispatcher {
    ...
    public static const ENABLED_ALARMS:Array = ["uuid4", "uuid6"];
...
//                    break;
    ...
    public function removeAlert(alarmUUID:String):void {
        ...
        if (_alertsContainer.length === 0 && prevLength != 0 && Statuses.instance.socketStatus) {
    ...
    public function hasAlert():Boolean {
        return _alertsContainer.length > 0;
    }


    private function infoTimer_timerHandler(event:TimerEvent):void {
        if (Statuses.instance.socketStatus && _alertsContainer.length === 0) {
            var aUUID:String
            var alarmX:Alarm;
            for (var i:int = 0; i < _infoContainer.length; i++) {
    ...
    private function validationTimer_timerHandler(event:TimerEvent):void {
        if (Statuses.instance.socketStatus) {
            ...
            for (var i:int = 0; i < _orderedAlarms.length; i++) {
                ...
                if (alarm is LimitAlarm && !(SailData.actualSailData[alarm.listenerKey.key] as BaseSailData).isValid()) {
                    alarm.isValid = false;
                }
