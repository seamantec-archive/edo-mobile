...
cp "${PROVISION_PATH}${PROFILE_NAME}.mobileprovision" "${TEMP}/Payload/${APP_NAME}.app/embedded.mobileprovision"

#Change BundleVersion
if [ ! -z ${APP_BUNDLE_VERSION} ]; then
    /usr/libexec/PlistBuddy -c "Set CFBundleVersion ${APP_BUNDLE_VERSION}" ${TEMP}/Payload/EDO\ Instruments\ Mobile.app/Info.plist
fi
